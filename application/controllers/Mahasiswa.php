...
class Mahasiswa extends CI_Controller {
	...
	public function createabsen(){
		...
		for ($i=0; $i < count($mk); $i++) { 
			...
			// $sql = $this->db->query("SELECT kelompok, COUNT(*) kel FROM tblabsensi  WHERE kodemk ='$mk[$i]' AND kelas = '$kls' GROUP BY kelompok HAVING kel = 5;");
			...
			// $last++;
			// $keltemp = $last; 
			// $sql1 = $this->db->query("SELECT kelompok, COUNT(*) ttl FROM tblabsensi WHERE kelompok = $last AND kodemk = '$mk[$i]'");
